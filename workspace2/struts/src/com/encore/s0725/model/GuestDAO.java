...
public class GuestDAO {
	...
	public List<Guest> selectAll() throws SQLException { // 리스트에 사용
		...
		/*
		 * List<Guest> list = smc.queryForList(String id, int skip, int max); ---> skip
		 * : 전체 조회 행 중에서 건너뛰기 할 행의 수 ---> max(=length) : 조회 할 최대 행 수
		 * 
		 * 글 번호 30 ~ 20 skip 10 ===> 글 번호 30부터 10개 skip!! ==> 글 번호 20번부터 조회 skip 20 ===>
		 * 글 번호 20부터 10개 skip!! ==> 글 번호 10번부터 조회 skip 30 ===> 글 번호 30부터 10개 skip!! ==>
		 * 글 번호 0번부터 조회
		 * 
		 * 1페이지 : 32 ~ 23 조회!! skip : 0, max : 10 1페이지 : 22 ~ 13 조회!! skip : 10, max :
		 * 10 1페이지 : 12 ~ 3 조회!! skip : 20, max : 10 1페이지 : 2 ~ 0 조회!! skip : 30, max :
		 * 10 page * 10 - 10
		 * 
		 * 
		 */
	...
	public boolean selectPassCheck(Map<String, Object> map) throws SQLException {
		Integer cnt = (Integer) smc.queryForObject("guest.selectPassCheck", map);
		if (cnt == 1)
			return true;
		return false;
	}
